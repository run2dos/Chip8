...
def OpcodeF(x):
  if (int(x, 16) & 0x00FF) == 0x0007:
    print(x, 'FX07 Sets VX to the value of the delay timer.')
  elif (int(x, 16) & 0x00FF) == 0x000A:
    print(x, 'FX0A A key press is awaited, and then stored in VX.')
  elif (int(x, 16) & 0x00FF) == 0x0015:
    print(x, 'FX15 Sets the delay timer to VX.')
  elif (int(x, 16) & 0x00FF) == 0x0018:
    print(x, 'FX18 Sets the sound timer to VX.')
  elif (int(x, 16) & 0x00FF) == 0x001E:
    print(x, 'FX1E Adds VX to I.')
  elif (int(x, 16) & 0x00FF) == 0x0029:
    print(x, 'FX29 Sets I to the location of the sprite for the character in VX. Characters 0-F (in hexadecimal) are represented by a 4x5 font.')
  elif (int(x, 16) & 0x00FF) == 0x0033:
    print(x, 'FX33 Stores the Binary-coded decimal representation of VX, with the most significant of three digits at the address in I, the middle digit at I plus 1, and the least significant digit at I plus 2. (In other words, take the decimal representation of VX, place the hundreds digit in memory at location in I, the tens digit at location I+1, and the ones digit at location I+2.)')
  elif (int(x, 16) & 0x00FF) == 0x0055:
    print(x, 'FX55 Stores V0 to VX in memory starting at address I.')
  elif (int(x, 16) & 0x00FF) == 0x0065:
    print(x, 'FX65 Fills V0 to VX with values from memory starting at address I.')
  else:
